...
var deleteBtn = ( '<div class="btn deleteItem">Delete</div>' );
var editBtn = ( '<div class="btn editItem">Edit</div>' );
...
function buildTable ( data ) {
	for (i = 0; i < data.length; i++) {
		$( '#objectBody' ).append( '<tr class="itemRow" id="' + (data[i]._id) + '">' + '<td class="data" id="trackName">' + (data[i].track) + '</td>' + '<td class="data" id="artistName">' + (data[i].artist) + '</td>' + '<td class="data" id="albumName">' + (data[i].album) + '</td>' + '<td class="data" id="releaseName">' + (data[i].release) + '</td>' + '<td class="btn">' + deleteBtn + '</td>' + '<td class="btn">' + editBtn + '</td>' + '</tr>' );
	};
...
function deleteThis () {
	var thisID = $(this).parent().parent().attr('id');
	var thisTrack = $(this).parent().siblings("#trackName").text();
	var r = confirm('Delete track info for ' + thisTrack + '?');
	if (r == true) {
		$.ajax( baseURL + collection + '/' + thisID,
		{
			method: 'DELETE',
